...
#include "FloatPackingUtils.hlsl"
#include "BC1Tables.hlsl"

#define COLOR_TYPE uint 
#define WRITE_COLOR(x) (packUnorm4x8((x)))
#define READ_COLOR(x)  (unpackUnorm4x8((x)))

#define FLT_MAX 340282346638528859811704183484516925440.0f

#define p_numRefinements (16)

float3 rgb565to888( float rgb565 )
{
	float3 retVal;
	retVal.x = floor( rgb565 / 2048.0f );
	retVal.y = floor( fmod( rgb565, 2048.0f ) / 32.0f );
	retVal.z = floor( fmod( rgb565, 32.0f ) );
...
bool RefineBlock( uint srcPixelsBlock[16], uint mask, inout float inOutMinEndp16,
				  inout float inOutMaxEndp16 )
{
	...
	{
		float w1Tab[4] = { 3, 0, 2, 1 };
		float prods[4] = { 589824.0f, 2304.0f, 262402.0f, 66562.0f };
...
void DitherBlock( uint srcPixBlck[16], out uint dthPixBlck[16] )
{
...
void CompressTextureBC1(Texture2D input, RWTexture2D<uint2> output, int2 threadID)
{
	COLOR_TYPE srcPixelsBlock[16];
    bool bAllColoursEqual = true;

    uint2 pixelsToLoadBase = threadID.xy * 4;

    for( uint i = 0u; i < 16u; ++i )
	{
		uint2 pixelsToLoad = pixelsToLoadBase + uint2( i & 0x03u, i / 4 );
		float3 srcPixels0  = input.Load(int3(pixelsToLoad, 0)).xyz;

		srcPixelsBlock[i]         = WRITE_COLOR( float4( srcPixels0, 1.0f ) );
		bAllColoursEqual          = bAllColoursEqual & srcPixelsBlock[0] == srcPixelsBlock[i];
	}
    
    float maxEndp16 = 0.0, minEndp16 = 0.0;
	uint  mask = 0u;

    UNITY_BRANCH
    if( bAllColoursEqual )
	{
		uint3 rgbVal = (uint3)(unpackUnorm4x8( srcPixelsBlock[0] ).xyz * 255.0);
		mask      = 0xAAAAAAAAu;
		maxEndp16 = c_oMatch5[rgbVal.r][0] * 2048.0f + c_oMatch6[rgbVal.g][0] * 32.0 + c_oMatch5[rgbVal.b][0];
		minEndp16 = c_oMatch5[rgbVal.r][1] * 2048.0f + c_oMatch6[rgbVal.g][1] * 32.0 + c_oMatch5[rgbVal.b][1];
	}
    else 
    {
#if defined (BC1_DITHER)
    ...
    output[threadID.xy] = outputBytes;
}